...
## Multitenency
As of now, **MULTITENENCY IS NOT SUPPORTED** by this wrapper which means you will not be able to use this wrapper in a single application with multiple Instamojo accounts. The call to `ApiContext.create()` returns a singleton object of class `ApiContext` with the given CLIENT_ID and CLIENT_SECRET, and will always return the same object even when called multiple times (even with a different CLIENT_ID and CLIENT_SECRET).
